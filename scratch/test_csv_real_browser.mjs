...
async function fetchFromNSE(page, optionType, fromDate, toDate, isCsv = false) {
  let url = `/api/historicalOR/bulk-block-short-deals?optionType=${optionType}&from=${fromDate}&to=${toDate}`;
  if (isCsv) {
    url += '&csv=true';
  }
...
async function main() {
  ...
  try {
    ...
    // Block ONLY images (exactly like v9)
    ...
    const jsonRes = await fetchFromNSE(page, 'bulk_deals', '15-05-2026', '15-05-2026', false);
    if (jsonRes.json) {
    ...
    const csvSingleRes = await fetchFromNSE(page, 'bulk_deals', '15-05-2026', '15-05-2026', true);
    if (csvSingleRes.text) {
      ...
      console.log(`SUCCESS. CSV Saved to ${filePath}. Total Lines: ${lines.length}`);
    ...
    const csvRangeRes = await fetchFromNSE(page, 'bulk_deals', '15-05-2026', '22-05-2026', true);
    if (csvRangeRes.text) {
