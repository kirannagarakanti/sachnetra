...
async function main() {
  try {
    ...
    const oldUrl = 'https://www.nseindia.com/api/option-chain-indices?symbol=NIFTY';
    console.log(`[Fetch] Fetching old endpoint ${oldUrl}...`);
    
    // Set a timeout of 10s using an AbortController so it doesn't hang indefinitely
    ...
    console.error('Error fetching old endpoint:', err.message);
  }
}
