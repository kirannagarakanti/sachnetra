...
| 2026-05-28 | **Exp 12 (cointegration pairs) + Exp 13 (rolling Hurst) run against prod** — two parallel-track experiments while Exp 11 latency is gated on V2-031b coverage. **Exp 12 🟡 PROMISING:** 4 same-sector Nifty-50 pairs cointegrated in-sample (HDFCBANK/KOTAKBANK best, EG t=−4.97) but **none OOS-tradeable** (reversion ~50%, spread drifts); Tier-B 179/1,035 pass raw 5% vs ~52 chance yet 0 survive BH-FDR. **Exp 13 🟡 WEAK DRIFT:** Nifty **persistently trends** (mean H=0.571, 66% of days >0.55), index-level mean-reversion near-absent (0.1%). Combined read: **mean-reversion is not a near-term harvestable edge on this universe** — pairs parked on a watchlist, MS-AR (§D5) parked. Added discipline rules B1–B4 to the playbook. H12/H13 logged. |
| 2026-05-28 | **Exp 14 (governance-shock event study) run against prod** — Top recommendation #1 from `india_proven_strategies_landscape` §4 (mid-cap governance-shock arbitrage). Verdict **⬜ BLOCKED**: of 712 auditor/pledge filings in the 5-week window only **9** land on a priced Nifty-50 symbol and just **2** survive to a usable ±5d window — both benign auditor *appointments* (DRREDDY 2026-05-12, CIPLA 2026-05-13), classified `auditor_change` **not** resignations, POST +411 bps (wrong sign; degenerate N=2 t). Both primary buckets (auditor_resignation, pledge_increase) **N=0**. NOT a market verdict — data starvation, exactly as Exp 2 predicted (96%+ of governance filings are unpriced mid/small-caps; survivorship). Re-run monthly; real unlocks = **V2-031c** (broaden ticker tagging → more priced symbols) and **G4** (Nifty Midcap 150 price universe). Do NOT paper-trade. H14 logged in playbook register. |
